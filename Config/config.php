...
class ECOMM
{
    ...
    {
        $userId = $this->getUser()['id'];
        $query = "SELECT * FROM `cart` WHERE userId=$userId && product_id=$id";
        // SELECT * FROM `cart` WHERE `userId`=6 && product_id=2; 
        // $result = $this->conn->query($query);
        return $this->conn->query($query)->num_rows ?? $this->conn->error;
    }
    ...
    public function prPageNos($tbl, $pn, $limit = 5, $catId = '')
    {
        // $total_pages = ceil($this->conn->query("SELECT COUNT(*) FROM $tbl")->fetch_row()[0] / $limit);
        ...
        if ($catId != '') {
            $total_pages = ceil($this->conn->query("SELECT COUNT(*) FROM $tbl WHERE categoryId=$catId")->fetch_row()[0] / $limit);

            for ($i = 1; $i <= $total_pages; $i++) {
                if ($i == $pn) {
                    $pagLink .= "<a class='activeLink' href='?catId=" . $catId . "&page=" . $i . "'>" . $i . "</a>";
                } else {
                    $pagLink .= "<a class='' href='?catId=" . $catId . "&page=" . $i . "'>" . $i . "</a>";
                }
            }
        } else {
            $total_pages = ceil($this->conn->query("SELECT COUNT(*) FROM $tbl")->fetch_row()[0] / $limit);
